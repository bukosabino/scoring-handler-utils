...
class ManagerProfile:
    ...
    @staticmethod
    def _prepare_output_path(path_profile, filename):
...
class ManagerProfilePyinstrument(ManagerProfile):
    ...
    def stop_and_write(self, path_profile, is_docker: bool):
        self.profiler.stop()

        if not is_docker:
            output_html = self.profiler.output_html()
            self._write_output_file(
                path_profile, output_html, filename="pyinstrument_profile_async.html"
            )

        print(self.profiler.output_text(unicode=True, color=True))

    def _write_output_file(self, path_profile, output_html, filename):
...
class ManagerProfileYappi(ManagerProfile):
    ...
    def stop_and_write(self, path_profile, is_docker: bool):
        ...
        if not is_docker:
            self._write_output_file(
                path_profile, stats, filename="yappi_profile_async.txt"
            )
        stats.print_all()

    def _write_output_file(self, path_profile, stats, filename):
